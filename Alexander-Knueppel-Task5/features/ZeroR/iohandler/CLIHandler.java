...
public class CLIHandler extends Handler {
	...
	public static String CLASSIFIER_ZEROR_COMMAND = "ZeroR";
	
	private void addClassifiers(ArrayList<String> a) {
		original(a);
		a.add(CLIHandler.CLASSIFIER_ZEROR_COMMAND);
	}
	
	private boolean classify(String identifier, int catIndex) {
		boolean res = original(identifier, catIndex);
		if(identifier.trim().toLowerCase().equals(CLIHandler.CLASSIFIER_ZEROR_COMMAND.toLowerCase())) {
			Classifier c = new ZeroRClassifier(catIndex);
